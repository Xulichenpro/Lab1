...
from cs336_basics import block,train_utils,tokenizer
...
def run_linear(
    ...
    linear = block.linear_block.Linear(
        d_in,
        d_out,
    )
...
def run_embedding(
    ...
    embedding = block.embedding_block.Embedding(
        vocab_size,
        d_model,
    )
    state_dict = {"weight":weights}
...
def run_swiglu(
    ...
    swiglu = block.swiglu_block.SWIglu(d_model,d_ff)
    swiglu.linear_unit1.weight.data = w1_weight
    swiglu.linear_unit2.weight.data = w2_weight
    swiglu.linear_unit3.weight.data = w3_weight
...
def run_scaled_dot_product_attention(
    ...
    """
    return block.attention_block.scaled_dot_product_attention(Q,K,V,mask)
...
def run_rope(
    ...
    """
    rope = block.rope_block.RoPE(theta,d_k,max_seq_len)
...
def run_rmsnorm(
    ...
    """
    rms = block.rms_norm_block.RMSNorm(
        d_model,
        eps = eps,
    )
    state_dict = {"weight":weights}
...
def run_get_batch(
    ...
    """
    return train_utils.dataloader.data_loader(dataset,batch_size,context_length,device)
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    return block.attention_block.softmax(in_features,dim)
...
def run_cross_entropy(
    ...
    """
    return train_utils.loss_fn.cross_entropy(inputs,targets)
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    train_utils.optimizer.grad_clipping(parameters,max_l2_norm)
...
def get_adamw_cls() -> Any:
    ...
    """
    return train_utils.optimizer.AdamW
...
def run_get_lr_cosine_schedule(
    ...
    """
    return train_utils.optimizer.learning_rate_schedule(it,max_learning_rate,min_learning_rate,warmup_iters,cosine_cycle_iters)
...
def run_save_checkpoint(
    ...
    """
    train_utils.checkpoint_utils.save_checkpoint(model,optimizer,iteration,out)
...
def run_load_checkpoint(
    ...
    """
    return train_utils.checkpoint_utils.load_checkpoint(src,model,optimizer)
...
def get_tokenizer(
    ...
    """
    return tokenizer.tokenizer.Tokenizer(vocab,merges,special_tokens)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    return tokenizer.train.train_bpe(input_path,vocab_size,special_tokens)
